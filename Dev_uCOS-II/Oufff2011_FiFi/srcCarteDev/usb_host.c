...
//******************************************************************************
    ...
    BDT_ENTRY __attribute__ ((aligned(512)))    BDT_HOST[2];
    #define BDT_IN                                  (&BDT_HOST[0])           // EP0 IN Buffer Descriptor
    #define BDT_OUT                                 (&BDT_HOST[1])           // EP0 OUT Buffer Descriptor
#elif (USB_PING_PONG_MODE == USB_PING_PONG__EP0_OUT_ONLY)
    BDT_ENTRY __attribute__ ((aligned(512)))    BDT_HOST[3];
    #define BDT_IN                                  (&BDT_HOST[0])           // EP0 IN Buffer Descriptor
    #define BDT_OUT                                 (&BDT_HOST[1])           // EP0 OUT Even Buffer Descriptor
    #define BDT_OUT_ODD                             (&BDT_HOST[2])           // EP0 OUT Odd Buffer Descriptor
#elif (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)
    BDT_ENTRY __attribute__ ((aligned(512)))    BDT_HOST[4];
    #define BDT_IN                                  (&BDT_HOST[0])           // EP0 IN Even Buffer Descriptor
    #define BDT_IN_ODD                              (&BDT_HOST[1])           // EP0 IN Odd Buffer Descriptor
    #define BDT_OUT                                 (&BDT_HOST[2])           // EP0 OUT Even Buffer Descriptor
    #define BDT_OUT_ODD                             (&BDT_HOST[3])           // EP0 OUT Odd Buffer Descriptor
#endif
        ...
        case STATE_DETACHED:
            switch (usbHostState & SUBSTATE_MASK)
            {
                case SUBSTATE_INITIALIZE:
                    ...
                    U1IR                = 0xFF;		// USB Interrupt Status,Master
//                    U1OTGIE             = 0;		// USB OTG Interrupt Enable,remove Sure Electronics
//                    U1OTGIR             = 0xFF;		// USB OTG Interrupt Status,remove Sure Electronics
                    U1EIE               = 0;		// USB Error Interrupts Enable
                    U1EIR               = 0xFF;		// USB Error Interrupts Register
                    U1IEbits.ATTACHIE   = 1;        // Enable the ATTACH interrupt.

                    // Initialize the Buffer Descriptor Table pointer.
                    #if defined(__C30__)
                       U1BDTP1 = (WORD)(&BDT_HOST) >> 8;
                    #elif defined(__PIC32MX__)
                       U1BDTP1 = ((DWORD)KVA_TO_PA(&BDT_HOST) & 0x0000FF00) >> 8;
                       U1BDTP2 = ((DWORD)KVA_TO_PA(&BDT_HOST) & 0x00FF0000) >> 16;
                       U1BDTP3 = ((DWORD)KVA_TO_PA(&BDT_HOST) & 0xFF000000) >> 24;
                    #else
                    ...
                    U1OTGCON            = USB_DPLUS_PULLDOWN_ENABLE | USB_DMINUS_PULLDOWN_ENABLE | USB_VBUS_ON ; // Pull down D+ and D-,change Sure Electronics
...
/****************************************************************************
  Function:
    void USB_Master_Interrupt( void )

...
static OS_STK       AppTaskDetectionStk[APP_TASK_DETECTION_STK_SIZE];
...
int  main (void)
{
	...
	Set_Line_Information( 2, 0, "    Coupe 2014   ", 16);
#endif
...
#ifdef APP_TASK_DETECTION_ENABLED
	// --------------------------------------------------------------------------------------------
	// Starts TaskDetection
    OSTaskCreateExt(TaskDetection_Main,                                       
				    (void *)0,
                    (OS_STK *)&AppTaskDetectionStk[APP_TASK_DETECTION_STK_SIZE - 1],
                    APP_TASK_DETECTION_PRIO,
                    APP_TASK_DETECTION_PRIO,
                    (OS_STK *)&AppTaskDetectionStk[0],
                    APP_TASK_DETECTION_STK_SIZE,
                    (void *)0,
                    OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR);

#if OS_TASK_NAME_SIZE > 13
    OSTaskNameSet(APP_TASK_DETECTION_PRIO, "TaskDetection", &err);
#endif
#endif

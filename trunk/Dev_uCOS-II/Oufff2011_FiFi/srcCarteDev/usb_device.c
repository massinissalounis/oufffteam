 ...
 *******************************************************************/
    ...
    volatile BDT_ENTRY BDT_DEVICE[(USB_MAX_EP_NUMBER + 1) * 2] __attribute__ ((aligned (512)));
#elif (USB_PING_PONG_MODE == USB_PING_PONG__EP0_OUT_ONLY)
    volatile BDT_ENTRY BDT_DEVICE[((USB_MAX_EP_NUMBER + 1) * 2)+1] __attribute__ ((aligned (512)));
#elif (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)
    volatile BDT_ENTRY BDT_DEVICE[(USB_MAX_EP_NUMBER + 1) * 4] __attribute__ ((aligned (512)));
#elif (USB_PING_PONG_MODE == USB_PING_PONG__ALL_BUT_EP0)
    volatile BDT_ENTRY BDT_DEVICE[((USB_MAX_EP_NUMBER + 1) * 4)-2] __attribute__ ((aligned (512)));
#else
...
void USBDeviceInit(void)
{
    ...
    for(i=0;i<(sizeof(BDT_DEVICE)/sizeof(BDT_ENTRY));i++)
    {
        BDT_DEVICE[i].Val = 0x00;
    }
    ...
    pBDTEntryIn[0] = (volatile BDT_ENTRY*)&BDT_DEVICE[EP0_IN_EVEN];
...
#endif
{
    ...
    if(USBResetIF && USBResetIE)
    {
        ...
        ********************************************************************/
        BDT_DEVICE[EP0_OUT_EVEN].ADR = (BYTE*)ConvertToPhysicalAddress(&SetupPkt);
        BDT_DEVICE[EP0_OUT_EVEN].CNT = USB_EP0_BUFF_SIZE;
        BDT_DEVICE[EP0_OUT_EVEN].STAT.Val &= ~_STAT_MASK;
        BDT_DEVICE[EP0_OUT_EVEN].STAT.Val |= _USIE|_DAT0|_DTSEN|_BSTALL;
...
void USBCtrlEPService(void)
{
    ...
    if((USTATcopy & USTAT_EP0_PP_MASK) == USTAT_EP0_OUT_EVEN)
    {
        ...
        #if defined(__18CXX)
            pBDTEntryEP0OutCurrent = (volatile BDT_ENTRY*)&BDT_DEVICE[(USTATcopy & USTAT_EP_MASK)>>1];
        #elif defined(__C30__) || defined(__C32__)
            pBDTEntryEP0OutCurrent = (volatile BDT_ENTRY*)&BDT_DEVICE[(USTATcopy & USTAT_EP_MASK)>>2];
        #else
            #error "unimplemented"
        #endif
...
void USBConfigureEndpoint(BYTE EPNum, BYTE direction)
{
    ...
    handle = (volatile BDT_ENTRY*)&BDT_DEVICE[EP0_OUT_EVEN];
...
void USBStallEndpoint(BYTE ep, BYTE dir)
{
    ...
    {
        p = (BDT_ENTRY*)(&BDT_DEVICE[EP(ep,dir,0)]);
        ...
        p = (BDT_ENTRY*)(&BDT_DEVICE[EP(ep,dir,1)]);
        p->STAT.Val |= _BSTALL | _USIE;
        #endif
    }
}
